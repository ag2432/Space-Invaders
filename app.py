...
class Invaders(GameApp):
    ...
    #
    # Attribute _score: the current score
    # Invariant: it should be an int
    #
    # Attribute _last: the current score
    # Invariant: it should be an int
    ...
    def start(self):
        ...
        self._text= GLabel(text="Press S to continue",font_size=24, left=300, \
        bottom=350)
        self._wave= None
        self._score= 0
    ...
    def update(self,dt):
        ...
        if self._state == STATE_ACTIVE:
            self._wave.update(dt, userinput=self.input)
            self.press_m()
            self.press_v()
        if self._state == STATE_PAUSED:
            self._text= GLabel(text="Press S to continue", font_size=24, \
            left=300, bottom=350)
            self.press_s()
        ...
        if self._state == STATE_COMPLETE:
            if self._wave.getShip() is None or \
            self._wave.aliensatdline() == True:
                self._text= GLabel(text="YOU LOST!", font_size=50, left=275, \
                bottom=350)
            if self._wave.getNumaliensalive() == 0:
                self._text= GLabel(text="YOU WON!", font_size=50, left=275, \
                bottom=350)
    ...
    def press_s(self):
        """
        Recognizes that 's' key has been pressed, removes the message,
        ...
        if change and self.input.is_key_down('s') and \
        self._state == STATE_INACTIVE:
    ...
    def press_m(self):
        """
        Recognizes that 'm' key has been pressed, mutes the sound.
        """
        if self.input.is_key_down('m'):
            self._wave.setAlienfireSound(0)
            self._wave.setAliencollisionSound(0)
            self._wave.setShipfireSound(0)
            self._wave.setShipcollisionSound(0)

    def press_v(self):
        """
        Recognizes that 'v' key has been pressed, and unmutes the sound.
        """
        if self.input.is_key_down('v'):
            self._wave.setAlienfireSound(1)
            self._wave.setAliencollisionSound(1)
            self._wave.setShipfireSound(1)
            self._wave.setShipcollisionSound(1)
    ...
    def _determineState(self):
        ...
        """
        if self._wave is not None:
            if self._wave.getLives() != 0 and self._wave.getShip() is None:
                self._state = STATE_PAUSED

            if self._wave.getShip() is None and self._wave.getLives() == 0:
                self._state = STATE_COMPLETE

            if self._wave.getNumaliensalive() == 0 or \
            self._wave.aliensatdline() == True:
                self._state = STATE_COMPLETE
